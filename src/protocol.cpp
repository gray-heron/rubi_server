...
void ProtocolHandler::rubi_inbound(CanRxMsg rx)
{
    ...
    rx.StdId == RUBI_BROADCAST1)
  {
    ...
    if ((rx.Data[0] & RUBI_MSG_MASK) != RUBI_MSG_BLOCK) {
      ...
      if (!transfer_failed) {
        rubi_data_outwrapper(
          rx.Data[0] & RUBI_MSG_MASK, rx.Data[1],
...
void ProtocolHandler::rubi_data_outwrapper(
  uint8_t msg_id, uint8_t id,
...
void ProtocolHandler::can_send_array(
  uint16_t cob, int32_t size,
  const uint8_t * data)
{
  std::vector<uint8_t> vdata;
  vdata.resize(size);
...
void ProtocolHandler::SendFFData(
  uint8_t ffid, uint8_t fftype,
...
void ProtocolHandler::SendCommand(
  uint8_t command_id,
  const std::vector<uint8_t> & data)
{
  ASSERT(data.size() < 7);
  ...
  can_send_array(
    RUBI_ADDRESS_RANGE1_LOW + board_nodeid,
